...
#include "test_gcrypt_h/config.h.in"
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
#include <sys/types.h>
...
#define PGM "gcrypt-benchmark"

#include "test_gcrypt_h/t-common.h"
...
md_bench(const char *algoname) {
    ...
    if (!algoname) {
        for (i = 1; i < 400; i++)
            if (in_fips_mode && i == GCRY_MD_MD5); /* Don't use MD5 in fips mode.  */
...
mac_bench(const char *algoname) {
    ...
    if (!algoname) {
        for (i = 1; i < 600; i++)
            if (in_fips_mode && i == GCRY_MAC_HMAC_MD5); /* Don't use MD5 in fips mode.  */
...
static void
uadk_md_bench(const char *algoname) {
    int algo;
    gcry_md_hd_t hd;
    int i, j, repcount;
    char *buf_base;
    ...
    gcry_error_t err = GPG_ERR_NO_ERROR;
    int times = 1000;
    //512B、1KB、10KB、512KB、1MB、10MB、20MB
    int test_data_size[7] = {512 + 15, 1024 + 15, 1024 * 10 + 15, 1024 * 512 + 15, 1024 * 1024 * 1 + 15,
                             1024 * 1024 * 10 + 15,
                             1024 * 1024 * 20 + 15};
    ...
    if (!algoname) {
        for (i = 1; i < 400; i++)
            if (in_fips_mode && i == GCRY_MD_MD5); /* Don't use MD5 in fips mode.  */
                ...
                uadk_md_bench(gcry_md_algo_name(i));
        return;
    }
    algo = gcry_md_map_name(algoname);
    if (!algo) {
    ...
    for (int temp = 0; temp < sizeof(test_data_size) / sizeof(int); temp++) {
        ...
        buf_base = (char *) malloc(sizeof(char) * bufsize);
        if (!buf_base) {
