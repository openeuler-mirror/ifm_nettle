...
showhex (const void *buffer, size_t buflen, const char *format, ...)
{
  ...
  for (s=(const unsigned char *)buffer; buflen; buflen--, s++)
...
hex2buffer (const char *string, size_t *r_length)
{
  ...
  buffer = (unsigned char *)xmalloc (strlen(string)/2+1);
  length = 0;
  for (s=string; *s; s +=2 )
    {
            ...
            int algo, int gigs, int bytes)
{
  ...
  tv_digest = (unsigned char *)hex2buffer ((char*)testvectors[idx].hex, &tv_digestlen);
