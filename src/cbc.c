...
void ifm_nettle_cbc_encrypt(const void *ctx, nettle_cipher_func *f, size_t block_size, uint8_t *iv, size_t length,
                            uint8_t *dst, const uint8_t *src)
{
    ...
    struct ifm_sm4_ctx *ctx_sm4 = NULL;
    ...
    if (f == (nettle_cipher_func *)ifm_aes128_encrypt) { // NOLINT
        ...
        uadk_ctx->alg = WCRYPTO_CIPHER_AES;
    ...
    } else if (f == (nettle_cipher_func *)ifm_sm4_crypt) { // NOLINT
        ctx_sm4 = (struct ifm_sm4_ctx *)ctx;
        uadk_ctx = &(ctx_sm4->uadk_ctx);
        uadk_ctx->alg = WCRYPTO_CIPHER_SM4;
        uadk_key = ctx_sm4->uadk_key;
        key_len = SM4_KEY_SIZE;
        p_use_uadk = &(ctx_sm4->use_uadk);
    } else {
        cbc_encrypt(ctx, f, block_size, iv, length, dst, src);
        IFM_ERR("the func is invalid, use cbc_encrypt\n");
        return;
    }
    // UADK不支持处理长度为0的字符串
    if (*p_use_uadk == true && length > 0 && length < AES_MAX_BLOCK_SZ) {
        ...
        if (uadk_ctx->ctx == NULL) {
            ...
            ret = uadk_cipher_set_key(uadk_ctx, uadk_key, key_len);
            if (ret != 0) {
                *p_use_uadk = false;
                cbc_encrypt(ctx, f, block_size, iv, length, dst, src);
                return;
        ...
        uadk_do_cipher(uadk_ctx, iv, dst, src, length, true);
...
void ifm_nettle_cbc_decrypt(const void *ctx, nettle_cipher_func *f, size_t block_size, uint8_t *iv, size_t length,
                            uint8_t *dst, const uint8_t *src)
{
#ifdef __aarch64__
    // 根据回调函数名称确定ctx的类型
    ...
    if (*p_use_uadk == true && length > 0 && length < AES_MAX_BLOCK_SZ) {
        ...
        uadk_do_cipher(uadk_ctx, iv, dst, src, length, false);
        // 根据原有nettle中cbc.c的实现，在加密完之后，会将dst最后内容回填到iv中
        memcpy(iv, dst + length - block_size, block_size);
    } else {
        cbc_decrypt(ctx, f, block_size, iv, length, dst, src);
...
void ifm_nettle_cbc_aes128_encrypt(const struct ifm_aes128_ctx *ctx, uint8_t *iv, size_t length, uint8_t *dst,
                                   const uint8_t *src)
{
    ifm_nettle_cbc_encrypt(ctx, (nettle_cipher_func *)aes128_encrypt, AES_BLOCK_SIZE, iv, length, dst, src);
}

void ifm_nettle_cbc_aes192_encrypt(const struct ifm_aes192_ctx *ctx, uint8_t *iv, size_t length, uint8_t *dst,
                                   const uint8_t *src)
{
    ifm_nettle_cbc_encrypt(ctx, (nettle_cipher_func *)aes192_encrypt, AES_BLOCK_SIZE, iv, length, dst, src);
}

void ifm_nettle_cbc_aes256_encrypt(const struct ifm_aes256_ctx *ctx, uint8_t *iv, size_t length, uint8_t *dst,
                                   const uint8_t *src)
{
    ifm_nettle_cbc_encrypt(ctx, (nettle_cipher_func *)aes256_encrypt, AES_BLOCK_SIZE, iv, length, dst, src);
}

void ifm_nettle_cbc_sm4_encrypt(const struct ifm_sm4_ctx *ctx, uint8_t *iv, size_t length, uint8_t *dst,
                                const uint8_t *src)
{
    ifm_nettle_cbc_encrypt(ctx, (nettle_cipher_func *)sm4_crypt, SM4_BLOCK_SIZE, iv, length, dst, src);
}
