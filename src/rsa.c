 ...
 * 6.暂未实现sign_tr函数、pss格式相关函数、rsa加密解密函数、keypair生成高级函数
 */
...
int ifm_rsa_pkcs1_sign(const struct ifm_rsa_private_key *key, size_t length, const uint8_t *digest_info, mpz_t s)
{
    ...
    if (key->use_uadk) {
        ...
        if (uadk_pkcs1_rsa_digest_encode(s, key_size, length, digest_info)) {
            size_t size_s;
            size_s = mpz_sizeinbase(s, BYTES_SIZE);
            uint8_t *data = (uint8_t *)malloc(size_s);
            mpz_export(data, &size_s, 1, sizeof(uint8_t), 0, 0, s);
                         ...
                         const mpz_t signature)
{
    ...
    if (key->use_uadk) {
        ...
        mpz_t m;
        mpz_init(m);
        res = (uadk_pkcs1_rsa_digest_encode(m, key_size, length, digest_info) &&
...
{
    return rsa_compute_root((struct rsa_private_key *)key, x, m);
}
